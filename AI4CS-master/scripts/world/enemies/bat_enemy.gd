...
#Per salvataggio (id unico cosi GameState sa chi non spawnare)
...
var levelCounter = 0
#per estrarre nome del nemico che sarà poi il nome della challenge
...
func _ready() -> void:
	...
	#2. Non spawna se gia killato sessione precedente
...
func openHackGuiAndPause():
	...
	#salva posizoine personaggio utile per quando apri challenge almeno ritorni nel punto in cui eri
	...
	# prepara runtime challenge cosi risolvi problemi se utente si arrende durante challenge ed esce senza concluderla
	TempState.challenge_active = true
	TempState.pending_scene_id = scene_id
	TempState.pending_enemy_uid = name
	#passa nome challegne
