...
@onready var textFlagSubmitted = $NinePatchRect/vert/NinePatchRect/MarginContainer/HBoxContainer/LineEdit
@onready var wrongflag = $wrongFlagColor
@onready var Helpscene = $HelpScene
...
func _ready():
	#Apre descrizione challenge appena apre la challenge
	Helpscene.open(TempState.pending_challenge)
...
func _on_submit_flag_pressed() -> void:
	var submitted_flag = textFlagSubmitted.text.strip_edges()
	if submitted_flag == "":
		return

	var hashalo = HashFlag.compute(pending_challenge, submitted_flag)
	#METTI IF: se flag inserita è uguale ad hash nel file flag allora è giusta e quindi fai questo codice, altrimenti guarda ****

	if ListHashFlag.is_valid(pending_challenge, hashalo):
		# flag corretta
	...
	#se **** allora fai uscire un pannello con scritto "sbagliato" (scompare dopo 2 secondi) e non succede nulla, rimani nella challenge
		...
		# flag sbagliata
		print("Flag errata")
		wrongflag.visible = true
		await get_tree().create_timer(0.3).timeout
		wrongflag.visible = false
